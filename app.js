...
function renderFeeDetails(students, transactions, summaries) {
  ...
  serviceTx.forEach(t=>{const key=t.feeDetail||serviceDetail('',t.content);const b=breakdown.get(key)||{count:0,codes:new Set(),amount:0};b.count++;if(t.studentCode)b.codes.add(t.studentCode);if(t.matched)b.amount+=num(t.amount);breakdown.set(key,b);});
